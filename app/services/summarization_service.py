...
async def summarize_batch(chunks: list[Chunk]) -> str:
    text: str = ""
    for chunk in chunks:
...
async def summarize_all_chunks(chunks: list[Chunk], batch_size: int = 10) -> list[str]:
    tasks = [
        summarize_batch(chunks[i : i + batch_size])
        for i in range(0, len(chunks), batch_size)
    ]
    batch_summaries = await asyncio.gather(*tasks)
    return list(batch_summaries)

async def final_summary_answer(batch_summaries: list[str]) -> str:
